...
function _reader:message_repeated(key, message_type)
    ...
    for i=0,n-1 do
        ...
        tinsert(ret, setmetatable( m , _R_meta ))	-- sub message do not need gc
    end
    return ret
end
...
local function encode_message(CObj, message_type, t)
    local encoders = encode_type_cache[message_type]
    for k,v in pairs(t) do
        local func = encoders[k]
...
-- return field type and name
function field(typename, field)
    return c._env_type(P, typename, field)
end
